...
export const tutorServer = createSdkMcpServer({
  ...
  tools: [
    ...
    // 4. update_exercise — FIXED: direct SQLite query instead of self-HTTP-fetch
    tool(
      "update_exercise",
      ...
      async ({ projectId, sessionId, exerciseId, status, hint }) => {
        ...
        // Direct SQLite query instead of self-HTTP-fetch
        const currentExercise = getExercise(sessionId, exerciseId);
        if (currentExercise) {
          if (hint) {
            updates.hints = [...(currentExercise.hints || []), hint];
          }

          // Update the latest attempt's status directly in SQLite
          if (status && currentExercise.attempts.length > 0) {
            updateLatestAttemptStatus(exerciseId, status);
          }
        }
